...
class Data {
    ...
    update() {
        if (this.input.cam.loadedmetadata) {
            ...
            let that = this;
            this.analysis.detector.detect(this.output.video, function (error, results) {
...
class Person {
    ...
    //Is the styling for the object tracking
    drawRect() {
        strokeWeight(8);
        noFill();
        fill(this.color.levels[0], this.color.levels[1], this.color.levels[2], 0 * this.deltaSizePercentage);
        stroke(this.color);
...
function updatePersons(parent) {
    let comb = createCombinations(parent.output.persons.length, parent.analysis.detections.length);
    ...
    for (let i = 0; i < combinations.length; i++) {
        ...
        for (let j = 0; j < combinations[i].length; j++) {
            ...
            if (typeof a !== 'undefined' && typeof b !== 'undefined') {
                totalDistance += getDistance(parent.output.persons[a].centerPoint, parent.analysis.detections[b].centerPoint);
            }
        }
    ...
    if (parent.analysis.personRemoval.length > 0) {
        parent.analysis.personRemoval.sort(function (a, b) { return b - a });
        ...
        for (let i = 0; i < parent.analysis.personAddition.length; i++) {
            let p = new Person(parent.analysis.detections[parent.analysis.personAddition[i]]);
...
function updatePerson(combination, persons, parent) {
    if (typeof combination[0] === 'undefined') {
        ...
        parent.output.persons[combination[0]].adapt(parent.analysis.detections[combination[1]]);
    }
}
